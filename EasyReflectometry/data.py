...
def _load_orso(fname: Union[TextIO, str]) -> sc.Dataset:
    ...
    for i, o in enumerate(f_data):
        coords = {}
        coords[f'Qz{i}'] = sc.array(dims=[f'{o.info.columns[0].name}{i}'], values=o.data[:, 0], variances=np.square(o.data[:, 3]), unit=sc.Unit(o.info.columns[0].unit)) 
        try:
            ordinate = sc.array(dims=[f'{o.info.columns[0].name}{i}'], values=o.data[:, 1], variances=np.square(o.data[:, 2]), unit=sc.Unit(o.info.columns[1].unit))
        except TypeError:
            ordinate = sc.array(dims=[f'{o.info.columns[0].name}{i}'], values=o.data[:, 1], variances=np.square(o.data[:, 2]))
...
def _load_txt(fname: Union[TextIO, str]) -> sc.Dataset:
    ...
    data = {'R0': sc.array(dims=['Qz0'], values=f_data[:, 1], variances=np.square(f_data[:, 2]))}
    coords = {data['R0'].dims[0]: sc.array(dims=['Qz0'], values=f_data[:, 0], variances=np.square(f_data[:, 3]), unit=sc.Unit('1/angstrom'))}
