...
class Fitter:
    ...
    def __init__(self, model: Union[Model, List[Model]],
        ...
        if isinstance(model, Model):
            ...
            if isinstance(fit_func, list):
                raise TypeError('If the model is a single model, the interface should be single interface')
    ...
    def fit(self, data: sc.Dataset, method: str = 'least_squares', id = 0):
        ...
        for i, x in enumerate(result.x):
            id = refl_nums[i]
            new_data[f'R_{id}_model'] = sc.array(dims=[f'Qz_{id}'], values=self.easy_f._fit_functions[i](data.coords[f'Qz_{id}'].values))
            new_data[f'SLD_{id}'] = sc.array(dims=[f'z_{id}'], values=self.easy_f._fit_objects[i].interface.sld_profile()[1] * 1e-6, unit=sc.Unit('1/angstrom')**2)
            new_data[f'R_{id}_model'].attrs['model'] = sc.scalar(self.easy_f._fit_object.as_dict())
            new_data.coords[f'z_{id}'] = sc.array(dims=[f'z_{id}'],values=self.easy_f._fit_objects[i].interface.sld_profile()[0], unit=(1 / new_data.coords[f'Qz_{id}'].unit).unit)
        return new_data
