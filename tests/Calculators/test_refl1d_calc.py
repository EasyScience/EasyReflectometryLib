...
from easyReflectometryLib.Calculators.refl1d import Refl1d
...
class TestRefl1d(unittest.TestCase):
    ...
    def test_create_model(self):
        p = Refl1d()
        p.create_model()
        assert_equal(p.storage['model'], {'scale': 1, 'bkg': 0, 'dq': 0, 'items': []})

    def test_update_model(self):
        p = Refl1d()
        p.create_model()
        p.update_model('model', scale=2, bkg=1e-3, dq=2.0)
        assert_almost_equal(p.storage['model']['scale'], 2)
        assert_almost_equal(p.storage['model']['bkg'], 1e-3)
        assert_almost_equal(p.storage['model']['dq'], 2.0)
    ...
    def test_add_item(self):
        ...
        p.add_layer_to_item('B_layer', 'B_item')
        p.create_model()
        assert_equal(len(p.storage['model']['items']), 0)
        p.add_item('B_item')
        assert_equal(len(p.storage['model']['items']), 1)
        assert_equal(p.storage['model']['items'][0].name, 'B_item')
    ...
    def test_calculate(self):
        ...
        p.create_model()
        p.update_model('model', bkg=1e-7, dq=5.0)
